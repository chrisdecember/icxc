...
| v7.35 | CASTLE-ESCAPE triggered for bots already marching (marchWp >= 0): bots that drifted back into the castle zone after passing wp0 got marchWp reset to -1, re-targeted wp0, oscillated between wp0 and castle-escape endlessly | added `this.marchWp < 0` guard — castle-escape only fires before march initialization. Moral: escape handlers must be gated on state, not just position |
| v7.35 | tower wp0-to-wp1 gap of 26 tiles stranded bots in obstacle zone; DEEP-STUCK (600 ticks) fired before MARCH-RESET (800 ticks), resetting march state before the march could self-correct. Bots cycled: stuck → relogin → same spot → stuck | added intermediate wp (3210,3196) to halve the gap, lowered MARCH-RESET from 800→450 ticks so it fires before DEEP-STUCK, MARCH-FORCE sets lastProgressTick on walkToward success to suppress DEEP-STUCK for actively-marching bots. Moral: **failsafe thresholds must be ordered by severity** |
